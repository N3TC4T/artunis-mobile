    ...
    setupTypeInfo(test, maskPattern) {
        ...
        for (let i = 0; i < 15; i++) {
            ...
            i < 6
                ...
                : i < 8 ? (this.modules[i + 1][8] = mod) : (this.modules[this.moduleCount - 15 + i][8] = mod);
        }
        for (let i = 0; i < 15; i++) {
            const mod = !test && ((bits >> i) & 1) == 1;
            i < 8
                ? (this.modules[8][this.moduleCount - i - 1] = mod)
                : i < 9 ? (this.modules[8][15 - i - 1 + 1] = mod) : (this.modules[8][15 - i - 1] = mod);
        }
...
class QRCode extends Component {
        ...
        return (
            <View
                style={{
                    width: nWidth,
                    height: nHeight,
                    backgroundColor: oQRCode.isDark(row, col) ? colorDark : colorLight,
                }}
                key={col}
            />
        );
    ...
    renderRow(oQRCode, row, width, nCount, nWidth, nHeight, colorDark, colorLight) {
        ...
        return (
            <View style={{ flexDirection: 'row', height: nHeight, width }} key={row}>
                {items}
            </View>
        );
    }
    ...
    render() {
        ...
        return <View style={{ width, height }}>{rows}</View>;
    }
}
